...
{ 
	_grp = ([_houseLeft,round (_countUnits * 6 / 10),_pool,_sideEnemy,_trigger] call AF_fillHouse); 
...
{
	private ["_unit","_pos"];
	private _countCivs	= round (_markSizeCoef + random (_markSizeCoef / 2));
...
_animCount 		= round (_markSizeCoef / 4 + 1 + random (_markSizeCoef / 6));
