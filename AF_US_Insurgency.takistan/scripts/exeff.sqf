...
SHOCK_WAVE = {
	...
	_smoke1 setposasl _aslLoc;
	_smoke1 setParticleCircle [0, [0, 0, 0]];
	_smoke1 setParticleRandom [0, [8, 8, 2], [100, 100, 0], 0, 0.25, [0.05, 0.05, 0.05, 0.1], 0, 0];
	...
	_smoke2 setposasl _aslLoc;
	_smoke2 setParticleCircle [0, [0, 0, 0]];
	_smoke2 setParticleRandom [0, [8, 8, 2], [100, 100, 0], 0, 0.25, [0.05, 0.05, 0.05, 0.1], 0, 0];
	...
	_smoke3 setposasl _aslLoc;
	_smoke3 setParticleCircle [0, [0, 0, 0]];
	_smoke3 setParticleRandom [0, [8, 8, 2], [100, 100, 0], 0, 0.25, [0.05, 0.05, 0.05, 0.1], 0, 0];
	...
	_smoke4 setposasl _aslLoc;
	_smoke4 setParticleCircle [0, [0, 0, 0]];
	_smoke4 setParticleRandom [0, [8, 8, 2], [100, 100, 0], 0, 0.25, [0.05, 0.05, 0.05, 0.1], 0, 0];
...
_numberOfFragments = 200;
			...
			[[_pos] , "IED_ROCKS", true, false] spawn BIS_fnc_MP;
			if(_size ==3) then {
				[[_pos] , "IED_SMOKE_LARGE", true, false] spawn BIS_fnc_MP;
				_numberOfFragments = 300;
			} else {
				if(_size == 1) then {
					[[_pos] , "IED_SMOKE_SMALL", true, false] spawn BIS_fnc_MP;
					_numberOfFragments = 100;
				} else {
					if(_size == 2) then {
						[[_pos] , "IED_SMOKE_MEDIUM", true, false] spawn BIS_fnc_MP;
						_numberOfFragments = 200;
					};
				};
			};
			//[_pos, _numberOfFragments] spawn CREATE_FRAGMENTS;	
};
