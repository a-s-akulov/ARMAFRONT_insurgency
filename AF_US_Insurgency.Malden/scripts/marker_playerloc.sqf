...
findDisplay 12 displayCtrl 51 ctrlAddEventHandler ["Draw",
	"_this call {
		{
			private _pl = _x;
			if (alive _pl && ((side group _pl) isEqualTo side group player) && !isObjectHidden _pl) then {
				private _i = _pl call {
					private _i = format [""%1"",
						switch true do {
				...
				private _c = [([playerSide,false] call BIS_fnc_sideColor),[0.227,0.773,0,1]] select (groupId (group _pl) isEqualTo groupId (group player));
				private _s = 14;
				private _txt = ""["";
